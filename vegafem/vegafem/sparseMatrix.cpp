...
//for debug data output
#include <fstream>
using namespace std;
void SparseMatrix::output2file(const std::string &filename, const bool &out_super_mat)
{
  ofstream ofs(filename);
  ofs <<"numrows:"<< numRows << endl;
  for(int i = 0;i < numRows;++i)
  {
    ofs << rowLength[i] << endl;
    for(int j = 0;j < rowLength[i];++j)
      ofs << "(" << columnIndices[i][j] << "," << columnEntries[i][j] << ")";
    ofs << endl;
  }
  //ofs << "num submat:" << numSubMatrixIDs << endl;
  //ofs << "********submat:*************" << endl;
  ...
  //  for(int j = 0;j < numRows;++j)
  //  {
  //    for(int k = 0;k < subMatrixIndexLengths[i][j];++k)
  //      ofs << subMatrixIndices[i][j][k] << " ";
  //    ofs << endl;
  //  }
  //}
  //if(!out_super_mat) { ofs.close();return; }
  //ofs << "**********super mat************" << endl;
  //for(int i = 0;i < numRows;++i)
  //{
  //  ofs << superRows[i] << endl;
  //  for(int j = 0;j < rowLength[i];++j)
  //    ofs << superMatrixIndices[i][j] << ",";
  //  ofs << endl;
  //}
  ofs.close();
}
