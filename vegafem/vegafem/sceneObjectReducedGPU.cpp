...
void SceneObjectReducedGPU::Construct(int GPUMethod)
{
  try
  {
    ...
    else if (GPUMethod == 1)
    {
        ...
        //render_uUq->Construct(mesh, meshRender, r, modalMatrix->GetMatrix(), renderMode);  //MDF BY TL  --original
        render_uUq->Init(mesh, meshRender, r, modalMatrix->GetMatrix(), renderMode);         //MDF BY TL  --ADD
      #else
        throw 10;
      #endif
    }
