...
StVKIsotropicMaterial *stVKIsotropicMaterial = NULL;
int g_rr = 0;
...
enum massSpringSystemSourceType { OBJ, TETMESH, CUBICMESH, CHAIN, NONE } massSpringSystemSource;
enum deformableObjectType { STVK, COROTLINFEM, LINFEM, MASSSPRING, INVERTIBLEFEM, UNSPECIFIED } deformableObject;
enum invertibleMaterialType { INV_STVK, INV_NEOHOOKEAN, INV_MOONEYRIVLIN, INV_NONE }  invertibleMaterial;
enum solverType { IMPLICITNEWMARK, IMPLICITBACKWARDEULER, EULER, SYMPLECTICEULER, CENTRALDIFFERENCES, UNKNOWN } solver;
...
double * f_extRes = NULL; //ADD BY TL
  ...
  if ((!lockScene) && (!pauseSimulation) && (singleStepMode <= 1))
  {
    ...
    if (g_iLeftMouseButton) 
    {
      if (pulledVertex != -1)
      {
        ...
        // forceNeighborhoodSize = 5; 5 layers
    ...
    // timestep the dynamics  substepsPerTimeStep = 1
    int code = 0;
    for(int i=0; i<substepsPerTimeStep; i++)
    {
      code=integratorBase->DoTimestep();
      ...
      // average forceAssemblyTime over last "forceAssemblyBufferSize" samples >>const int forceAssemblyBufferSize = 50;
...
void initSimulation()
{
  ...
  deformableObject = UNSPECIFIED;  //MDF BY TL
  ...
  massSpringSystemSource = NONE;  //MDF BY TL
  ...
  f_extRes = (double*)calloc(3 * n, sizeof(double));
  ...
  if (deformableObject == INVERTIBLEFEM)
  {
    ...
    invertibleMaterial = INV_NONE;  //MDF BY TL
    ...
    switch (invertibleMaterial)
    {
      case INV_STVK:
      {
        stVKIsotropicMaterial = new StVKIsotropicMaterial(tetMesh, enableCompressionResistance, compressionResistance);
    ...
    isotropicMaterial = stVKIsotropicMaterial;
    // create the invertible FEM deformable model
    //IsotropicHyperelasticFEM * isotropicHyperelasticFEM;
    ...
    //G_IsotropicHyperelasticFEMForceModel<MATF> *g_isotropicHyperelasticFEMForceModel = new G_IsotropicHyperelasticFEMForceModel<MATF>(g_isotropicHyperelasticFEM);

    forceModel = isotropicHyperelasticFEMForceModel;
    //g_forceModel_ = g_isotropicHyperelasticFEMForceModel;
  }
